...
export const ActiveRides: React.FC = () => {
  ...
  const isVerifyLoading = loadingStates[`verify-${walletAddress}`] || false;

  return (
    <div className="glass-card rounded-2xl p-5 flex flex-col h-[520px] relative">
      {/* Tab Switchers */}
      <div className="flex border-b border-zinc-800/80 mb-5 flex-wrap">
        ...
        >
          <Car className="w-4 h-4" /> Driver Hub ({driverProfile ? availableRidesForDrivers.length : "0"})
          {activeTab === "driver" && (
      ...
      {errorMessage && (
        <div className="mb-4 p-3 bg-rose-500/10 border border-rose-500/25 rounded-xl flex items-start gap-2 text-rose-400 text-xs animate-slide-up shrink-0">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
        {activeTab === "passenger" ? (
          ...
          /* DRIVER TAB FLOW */
          !driverProfile ? (
            /* Unregistered Driver Promo Card */
            <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
              <div className="w-14 h-14 rounded-2xl bg-indigo-950/20 border border-indigo-500/20 text-indigo-400 flex items-center justify-center">
                <UserPlus className="w-7 h-7" />
              </div>
              <div className="max-w-sm">
                <h4 className="font-black text-white text-sm uppercase tracking-wide">Become a RideMesh Driver</h4>
                <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
                  Earn RIDE tokens by picking up passengers. Register your vehicle, license and identity metadata directly on Stellar ledger.
                </p>
              </div>
              <button
                onClick={() => setIsRegModalOpen(true)}
                className="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md shadow-indigo-500/10 flex items-center gap-2 active:scale-95 cursor-pointer"
              >
                <Sparkles className="w-4 h-4 text-cyan-300" /> Register Driver Profile
              </button>
            </div>
          ) : (
            /* Driver Profile Card */
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-black">Driver Identity Profile</span>
                    <h5 className="font-black text-white text-sm mt-0.5">{driverProfile.name}</h5>
                  </div>
                  {driverProfile.isVerified ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-md text-[10px] font-bold uppercase tracking-wider">
                      <ShieldCheck className="w-3.5 h-3.5" /> Verified
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-md text-[10px] font-bold uppercase tracking-wider">
                      <ShieldAlert className="w-3.5 h-3.5" /> Pending Verification
                    </span>
                  )}
                </div>

                <div className="grid grid-cols-2 gap-3 py-2 border-t border-zinc-900 text-[10px] md:text-xs">
                  <div>
                    <span className="text-zinc-500 block uppercase font-semibold">Vehicle</span>
                    <span className="font-medium text-zinc-300 mt-0.5 block">{driverProfile.vehicleType}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 block uppercase font-semibold">Plate Number</span>
                    <span className="font-medium text-zinc-300 mt-0.5 block font-mono">{driverProfile.vehicleNumber}</span>
                  </div>
                </div>

                {/* Simulated Verify action for admin test sandbox */}
                {!driverProfile.isVerified && isSandbox && (
                  <button
                    disabled={isVerifyLoading}
                    onClick={() => verifyDriver(walletAddress || "")}
                    className="w-full py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-zinc-950 hover:opacity-90 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 transition-opacity disabled:opacity-50"
                  >
                    {isVerifyLoading ? (
                      <>
                        <Loader2 className="w-3 h-3 animate-spin" /> Verifying Profile...
                      </>
                    ) : (
                      <>
                        <ShieldCheck className="w-3 h-3" /> Verify Account (Simulate Admin)
                      </>
                    )}
                  </button>
                )}
              ...
              <div className="space-y-3">
                <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-black block">Available Passenger Fares</span>
                
                {availableRidesForDrivers.length === 0 ? (
                  <div className="p-8 rounded-xl border border-zinc-900 bg-zinc-950/20 text-center">
                    <Car className="w-6 h-6 text-zinc-700 mx-auto mb-1.5" />
                    <p className="text-xs text-zinc-400 font-semibold">No open passenger fares available</p>
                    <p className="text-[9px] text-zinc-500 mt-0.5">Open another tab/passenger connection to book a ride.</p>
                  </div>
                ) : (
                  availableRidesForDrivers.map((ride) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          disabled={isAcceptLoading || !driverProfile.isVerified}
                          onClick={() => acceptRide(ride.id)}
                          className="w-full py-2.5 bg-gradient-to-r from-secondary to-indigo-600 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 hover:opacity-90 transition-all shadow-md disabled:opacity-50 active:scale-98 cursor-pointer"
                        >
                          {isAcceptLoading ? (
                            <>
      ...
      {/* RENDER REGISTRY MODAL CONTROLS */}
